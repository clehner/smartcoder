...
pickProgram = (path, cb) ->
  pickBody path.concat('Program'), cb
...
class CodeFile
  ...
  generate: (n) ->
    pickProgram [@language, 'Program'], (ngram) ->
  ...
  pickNode: (path) ->
    values = {}
    for key, valueType of syntax[type]
      values[key] = switch valueType
        when Array
          pickArray path.concat(key)
        when Object
          pickNodeOrNull path.concat(key, valueType), valueType
        when Boolean, Number, String
          pickValue path.concat(key), valueType
        else
          if typeof valueType == 'string'
            pickNode path.concat(key, valueType), valueType
          else if valueType instanceof 'Array'
            pickArray path.concat(key), valueType[0]
          else
...
generateCodeFile = (n) ->
  generateFileType (err, fileName, langName) ->
    ...
    codeFile = new CodeFile fileName, langName
    codeFile.generate n
